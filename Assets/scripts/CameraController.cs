...
{
    public float panplayerSpeed = 30f;
    ...
    void Update()
    {
        ...
        {
            transform.Translate(Vector3.forward * panplayerSpeed*Time.deltaTime,Space.World);
        }

        if (Input.GetKey(KeyCode.S) || Input.mousePosition.y < panBorder)//BAS
        {
            transform.Translate(Vector3.back * panplayerSpeed * Time.deltaTime, Space.World);
        }

        if (Input.GetKey(KeyCode.Q) || Input.mousePosition.x <  panBorder)//GAUCHE
        {
            transform.Translate(Vector3.left * panplayerSpeed * Time.deltaTime, Space.World);
        }

        if (Input.GetKey(KeyCode.D) || Input.mousePosition.x > Screen.width - panBorder)//DROITE
        {
            transform.Translate(Vector3.right * panplayerSpeed * Time.deltaTime, Space.World);
        }
